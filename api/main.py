    ...
    async def process_frames(self, client_id: str, pose_type: str):
        ...
        try:
            while client_id in self.active_connections:
                ...
                # 🧠 Only send feedback every 0.7 sec
                if not hasattr(self, "last_feedback_time"):
                    self.last_feedback_time = 0
                if current_time - self.last_feedback_time > 0.7:
                    send_feedback = True
                    self.last_feedback_time = current_time

                # 🎥 Always send frame (to keep camera smooth)
                data_to_send = {"frame": frame_base64}

                # 🗣️ Only attach feedback occasionally
                if send_feedback:
                    data_to_send["feedback"] = {
                        "similarity": float(overall_sim),
                        "feedback_text": feedback_text,
                        "joint_similarities": joint_sims if isinstance(joint_sims, dict) else {}
                    }
                ...
                # 💤 Small sleep for stable frame rate
                await asyncio.sleep(0.03)

        except Exception as e:
            print(f"Error processing frames for client {client_id}: {e}")
        finally:
            if hasattr(self, "cap"):
                self.cap.release()
